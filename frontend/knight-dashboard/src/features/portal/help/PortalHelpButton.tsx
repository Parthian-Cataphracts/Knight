...
import { portalHelpForPath } from "./portalHelpContent";

/**
 * The portal's header "?" button — the merchant's counterpart to the operations
 * dashboard's help. It opens a side panel with the plain-language guide for the
 * page the merchant is on; the full index lives at /portal/help. The drawer is
 * rendered into document.body so a backdrop-filtered header cannot clip it.
 */
export function PortalHelpButton() {
  const [open, setOpen] = useState(false);
  const { pathname } = useLocation();
  const entry = portalHelpForPath(pathname);

  return (
    <>
      <button
        type="button"
        className="grid size-9 place-items-center rounded-md text-on-surface-variant hover:bg-surface-high hover:text-on-surface"
        aria-label="راهنما"
