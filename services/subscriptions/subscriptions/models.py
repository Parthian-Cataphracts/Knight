...
store for a payment method it holds. Nothing here is, or may become, a place a
card number could be stored — there is no field it would fit in and that is
deliberate.

---

**What changed when this stopped being a package.** In 1.x these tables lived in
each store's own database, so "this store's subscriptions" was the whole table.
This service holds one database for every store, and so **every row belongs to a
store and no query may cross that line**.

That is one field and one changed constraint, and both are load-bearing:
`Subscription.store`, and a reference that is unique *within* a store rather than
globally. Two shops both numbering their subscriptions from `SUB-1` is normal,
and a global unique index would have made the second shop's first subscription
fail to create (`adr/0033`).
"""
...
class Subscription(models.Model):
    ...
    #: The store this agreement belongs to. The field that makes one deployment
    #: able to serve many shops, and the field every query in `services.py` is
    #: scoped by. `PROTECT` rather than `CASCADE`: deleting a store row must not
    #: be a way to silently delete a merchant's billing history.
    ...
    #: Unique **within a store**, not globally. See the note at the top of this
    #: file: two shops both numbering from SUB-1 is the normal case.
    ...
    class Meta:
        ...
        ordering = ("-created_at",)
        constraints = [
            # What `unique=True` on the reference used to be, scoped. The
            # database is what stops one store reading another's subscription by
            # guessing a reference, because every lookup is (store, reference)
            # and this is what makes that pair identifying.
            models.UniqueConstraint(fields=["store", "reference"], name="knight_sub_ref_per_store"),
        ]
        indexes = [
            # The query the billing worker makes, and the only one that runs on a
            # timer against the whole table. Deliberately **not** led by store:
            # the worker bills every store's due subscriptions in one pass, so
            # leading with store would make it a scan per shop.
            models.Index(fields=["state", "next_run_at"], name="knight_sub_due"),
            models.Index(fields=["store", "source_shopper_id"], name="knight_sub_shopper"),
        ]
        constraints = [
