...
def create(
    ...
    *,
    ...
    existing = Subscription.objects.filter(reference=reference).first()
...
@transaction.atomic
def activate(reference: str, *, actor: str = "", now=None) -> Summary:
    """
    Starts the clock.
...
@transaction.atomic
def pause(reference: str, *, actor: str = "", reason: str = "", now=None) -> Summary:
    """
...
@transaction.atomic
def cancel(reference: str, *, actor: str = "", reason: str = "", now=None) -> Summary:
    """
    Ends the agreement.
...
def due(*, now=None, location: str | None = None):
    """
...
@transaction.atomic
def bill(reference: str, *, now=None, force: bool = False) -> Charge:
    """
...
def bill_due(*, now=None, limit: int = 500, location: str | None = None) -> dict[str, int]:
    ...
    for reference in list(due(now=now, location=location).values_list("reference", flat=True)[:limit]):
        try:
            charge = bill(reference, now=now)
...
def retry_failed(*, now=None, limit: int = 500) -> dict[str, int]:
    ...
    for period in periods:
        ...
        with transaction.atomic():
            locked = _locked(subscription.reference)
...
@transaction.atomic
def record_order(reference: str, sequence: int, order_number: int) -> SubscriptionOrder:
    """
    ...
    payment.
    """
    period = _period(reference, sequence)
...
def summarise(subscription: Subscription | str) -> Summary:
...
def history(reference: str) -> list[SubscriptionEvent]:
    """Everywhere a subscription has been, in order."""
    return list(_require(reference).events.all())


def periods(reference: str) -> list[BillingPeriod]:
    """Every period, paid or not, with its attempts."""
    return list(_require(reference).periods.prefetch_related("attempts").all())


def lines(reference: str) -> list[SubscriptionLine]:
    return list(_require(reference).lines.all())
...
def _locked(reference: str) -> Subscription:
    """
    ...
    """
    found = Subscription.objects.select_for_update().filter(reference=_reference(reference)).first()
...
def _require(reference: str) -> Subscription:
    found = Subscription.objects.filter(reference=_reference(reference)).first()
...
def _period(reference: str, sequence: int) -> BillingPeriod:
    found = BillingPeriod.objects.filter(
        subscription__reference=_reference(reference), sequence=int(sequence)
    ).first()
