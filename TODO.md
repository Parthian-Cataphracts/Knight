...
| **Current phase** | **Phase 29 — the production gate. One item was buildable and is built; the rest is yours.** Domain verification has had two methods and one implementation since phase 3; the DNS TXT half now exists, so a store with no HTTP surface yet can prove it owns its domain. The eleven product-owner answers are now given (2026-09-05, `docs/risks.md` §3); the remaining items are the security review, a production database, the backup-custody decision, and a release call — none of them code. The hosting platform is chosen: KNIGHT's own servers. [`docs/phase-29-verification.md`](docs/phase-29-verification.md) |
...
| **Next phase** | **None that this repository can start.** What is left across phases 27, 28 and 29 is: a domain, a backup-custody decision, three service conversions, and an engaged security reviewer. The host is decided — KNIGHT's own servers. The eleven product-owner answers in [`docs/risks.md`](docs/risks.md) §3 are given; Phonix is dropped (BojanStore is the first project). The two open decisions are in [`docs/roadmap.md`](docs/roadmap.md) §7 |
...
| **Blocking decisions** | R26 is **answered**: a Feature is publishable for any of three runtimes, and since phase 22 an `external_service` Feature needs no runtime at all. Most product-owner decisions are now made (2026-09-05): engage the security reviewer and close R16, service code lives in this repo, Phonix dropped for BojanStore. The hosting platform is now chosen too — **KNIGHT's own servers**. **One decision remains**, in [`docs/roadmap.md`](docs/roadmap.md) §7 — **backup custody** |
...
- [x] **Architecture validation by the product owner** — the 11 questions in `docs/risks.md` §3 are answered (2026-09-05). Settled: take payments + invoices; multi-currency with per-currency manual tax; centralised log ingestion (errors/warnings/alerts separated); support both company- and customer-managed agent hosting; English UI (**built** — dashboard is English-only, Persian bundle and every language switcher removed, `<html lang=en dir=ltr>`); provisioning per the proposal; immediate uninstall purge allowed; engage the security reviewer (close R16); Phonix dropped, BojanStore is the first project. Hosting platform chosen: KNIGHT's own servers. Still open: **backup custody** (`docs/roadmap.md` §7)
...
- [x] Log search, filtering by time and export — **delivered (2026-09-05), per the centralised-logs decision (`risks.md` §3.4).** `GET /api/v1/logs` now takes `minSeverity` (keeps everything at or above a severity — how the errors, warnings and alerts are separated from the noise, `LogSeverity` maps each store's raw tokens onto the ladder), `search` (case-insensitive `ILIKE` on the message, wildcards escaped so `50%` is literal), and `from`/`to` (a timestamp range), alongside the existing store and exact-level filters. `GET /api/v1/logs/export` returns the same filter as RFC 4180 CSV (formula-injection guarded, capped at 10k rows), behind a **`logs.export`** permission of its own — bulk egress of the least-redacted data — granted to Admin and Developer, held back from Support and customers. 5 unit + 5 integration tests. **Dashboard wired** (`features/logs/LogsPage.tsx`): severity-floor tabs, a `from`/`to` range, a full-text search and a permission-gated **Export CSV** button, all driving the server-side query rather than filtering a page already fetched. **Browser-verified against the live API** (mocks off): `minSeverity=Error` narrowed 200 rows to 3, `search=Payment` narrowed those to 1, and `/logs/export` returned the same filter as RFC 4180 CSV under the bearer token
...
- [x] **Immediate purge on request** — per the product-owner decision (`risks.md` §3.11, customers may ask to be purged at once). `ProvisioningJob.WaiveRetention` brings the retention window forward to now; `POST /api/v1/provisioning/{jobId}/purge-now` (needs `store.deprovision`, the same platform-only permission as starting the run, because it ends in deleted data) waives it and advances the run — the `export` step still writes the customer's copy before `purge`, so only the wait is removed. 3 unit + 2 integration tests
