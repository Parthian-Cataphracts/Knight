...
GET    /api/v1/logs?storeId=&level=&from=&to=&traceId=&q=
...
GET  /api/v1/ingest/features    store pulls its effective entitlements
...
Response: `202 Accepted` with `{ "accepted": 1, "rejected": 0, "groups": ["..."] }`.

Ingestion is rate-limited per store and rejects payloads whose `environment`
does not match the registered store environment.
