...
Claim a task by filling owner + branch + lease-until before editing its scope. Each
agent works on its OWN branch (`agent/<tool>-<task>`), never on main. A stale lease
(past its time) may be reclaimed — note it in `activity.md`. Statuses:
PLANNED → IN_PROGRESS → VERIFICATION_PENDING → VERIFIED (or FAILED). Merge to main
only after review (see AGENTS.md rule 4).

| id | task | scope (files/modules) | owner (tool) | branch | status | lease-until |
|----|------|-----------------------|--------------|--------|--------|-------------|
